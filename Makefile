...
PROJECT_NAME=intranet2-tremt
STACK_NAME=intranet2-tre-mt-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet2-tremt.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t lcpgeop/intranet2-tremt-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t lcpgeop/intranet2-tremt-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet2-tremt-frontend-acceptance --link intranet2-tremt-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d lcpgeop/intranet2-tremt-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet2-tremt-backend-acceptance -d lcpgeop/intranet2-tremt-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop intranet2-tremt-frontend-acceptance
	@docker stop intranet2-tremt-backend-acceptance
